"""How fast does a plan's representation go stale?  (copy of harvest_distill.py)

Stores, per correction, the teacher hidden state at plan time, the teacher
hidden state now, and the age between them.  Original docstring follows.
...
ap.add_argument("--task-trials", default="",
                help="per-task override of --trials, e.g. 5:120,7:120 (1-based task ids)")
...
buf = dict(jpg_a=[], jpg_w=[], state=[], lang=[], x=[], v=[], tau=[], meta=[],
           plan_feat=[], corr_feat=[], age=[])
n_pairs = 0
t0 = time.time()

TASK_TRIALS = {int(k) - 1: int(v) for k, v in (q.split(":") for q in args.task_trials.split(",") if q)}
for tid in range(n_tasks):
    for trial in range(args.trial_start, args.trial_start + TASK_TRIALS.get(tid, args.trials)):
        ...
        plan_dev = None        # the plan's hidden state, on device, for the driver
        K = args.k_draws
        while not w.terminated and steps < T_max:
            if A is None:
                w.mark_plan_anchor()
                A = be.plan(w.observe(), noise=torch.randn(H, D, generator=gen)).cpu()
                plan_feat = TAP["h"][0].to(torch.float16).cpu().clone()
                plan_dev = TAP["h"][0].float()
                ...
                corr_feat = TAP["h"][0].to(torch.float16).cpu().clone()
                buf["plan_feat"].append(plan_feat)
                buf["corr_feat"].append(corr_feat)
                buf["age"].append(used)
                ...
                if NET is not None:
                    vk = NET.velocity(xh[:K], tauh[:K],
                                      NET.memory(obs_now, plan=plan_dev),
                                      age=used).float()
                else:
...
torch.save(dict(
    ...
    plan_feat=torch.stack(buf["plan_feat"]), corr_feat=torch.stack(buf["corr_feat"]),
    age=torch.tensor(buf["age"]),
